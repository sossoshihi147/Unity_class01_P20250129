...
public class Class_04_Function : MonoBehaviour 
{
    private void Awake()
    {
        ...
        //BMI呼叫傳回練習
        Debug.Log($"<color=#fd0> BMI值 : {BMI(1.68f,60)}</color>");
        Debug.Log($"<color=#fd0> BMI值 : {BMI(1.62f,74)}</color>");
    ...
    //框選要整理得程式 > Ctrl + K S >#region 快速完成程式碼片段
    #region 方法基本語法與參數
    // 方法:
    // 包含一系列程式區塊
    // 方法語法:
    ...
    private void UseSkill(string skill, float cost)
    {
        ...
        Debug.Log($"<color=#0f0>技能消耗 : {cost}</color>");
    }
    ...
    {
        Debug.Log($"<color=#f0f>購買商品 : {item}，數量 : {count}</color>");
    }
    #endregion
    ...
    /// <returns></returns>
    private float BMI(float hight, float weight)
    {
        return weight / Square(hight);
    }
    #endregion
